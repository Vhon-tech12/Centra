            ...
            {tab === "treatment" && (
              ...
              >
                {!localPatient?.soapNote?.plan && !localPatient?.soapNote?.followUp ? (
                  <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/60 px-6 py-14 text-center">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-slate-700">
                        No next treatment details available
                      </p>
                      <p className="mt-1 text-xs text-slate-500">
                        The treatment plan and follow-up will appear here once recorded.
                      </p>
                    </div>
                  </div>
                ) : (
                  <div className="grid gap-4 print:gap-3">
                    {localPatient?.soapNote?.plan && (
                      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-teal-50 via-teal-50/60 to-white p-5 ring-1 ring-teal-100/80 transition hover:shadow-sm print:bg-white print:ring-0 print:border print:border-slate-200">
                        <div className="flex items-start gap-4">
                          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-teal-600 text-white shadow-sm print:h-8 print:w-8 print:shadow-none">
                            <svg className="h-5 w-5 print:h-4 print:w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
                              <path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2" />
                              <rect x="9" y="3" width="6" height="4" rx="1" />
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2">
                              <p className="text-xs font-bold uppercase tracking-wider text-teal-700 print:text-slate-600">
                                Plan
                              </p>
                              <span className="h-1 w-1 rounded-full bg-teal-300" />
                              <span className="text-[11px] font-medium text-teal-600/70 print:hidden">
                                Treatment plan
                              </span>
                            </div>
                            <p className="mt-2 text-sm font-medium leading-relaxed text-slate-800 whitespace-pre-line print:text-xs">
                              {localPatient.soapNote.plan || "—"}
                            </p>
                          </div>
                        </div>
                    ...
                    {localPatient?.soapNote?.followUp && (
                      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-50 via-indigo-50/60 to-white p-5 ring-1 ring-indigo-100/80 transition hover:shadow-sm print:bg-white print:ring-0 print:border print:border-slate-200">
                        <div className="flex items-start gap-4">
                          <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-indigo-600 text-white shadow-sm print:h-8 print:w-8 print:shadow-none">
                            <svg className="h-5 w-5 print:h-4 print:w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2">
                              <p className="text-xs font-bold uppercase tracking-wider text-indigo-700 print:text-slate-600">
                                Follow-up
                              </p>
                              <span className="h-1 w-1 rounded-full bg-indigo-300" />
                              <span className="text-[11px] font-medium text-indigo-600/70 print:hidden">
                                Scheduled follow-up
                              </span>
                            </div>
                            <p className="mt-2 text-sm font-medium leading-relaxed text-slate-800 whitespace-pre-line print:text-xs">
            ...
            {tab === "medical" && (
              <SectionCard
                ...
                right={
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-teal-600 to-teal-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:from-teal-700 hover:to-teal-800 hover:shadow-md print:hidden"
                  >
              ...
              >
                {loadingMedicalHistory ? (
                  <div className="flex flex-col items-center justify-center gap-3 rounded-2xl bg-slate-50/60 px-6 py-14 ring-1 ring-slate-200/60">
                    <svg className="h-6 w-6 animate-spin text-teal-600" viewBox="0 0 24 24" fill="none">
                      <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                      <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 0 1 8-8v4a4 4 0 0 0-4 4H4z" />
                    </svg>
                    <p className="text-sm font-medium text-slate-500">Loading medical histories...</p>
                  </div>
                ) : medicalHistoryError ? (
                  <div className="flex items-start gap-3 rounded-2xl bg-red-50 px-5 py-5 ring-1 ring-red-200/70">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-red-700">Couldn't load medical history</p>
                      <p className="mt-0.5 text-sm text-red-600">{medicalHistoryError}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-center justify-center gap-4 rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/60 px-6 py-14 text-center">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        Get started by creating the first medical history entry for this patient.
                      </p>
                    </div>
                    <button
                      onClick={onCreateMedicalHistory}
                      className="mt-1 inline-flex items-center gap-2 rounded-xl bg-teal-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-teal-700 print:hidden"
                    >
                      <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round">
                        <line x1="12" y1="5" x2="12" y2="19" />
                        <line x1="5" y1="12" x2="19" y2="12" />
                  ...
                  <div className="space-y-5 print:space-y-4">
                    {medicalHistories.map((record) => (
                      <div
                        key={record.id}
                        className="group relative overflow-hidden rounded-2xl bg-white ring-1 ring-slate-200/70 transition hover:shadow-md hover:ring-teal-200 print:shadow-none print:ring-0 print:border print:border-slate-200 print:break-inside-avoid"
                      >
                        {/* Left accent bar */}
                        <div className="absolute inset-y-0 left-0 w-1 bg-gradient-to-b from-teal-400 to-teal-600" />

                        <div className="p-6 pl-7 print:p-4 print:pl-5">
                          {/* Record header */}
                          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                            <div className="flex min-w-0 items-start gap-3">
                              <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-teal-50 text-teal-600 ring-1 ring-teal-100 print:h-8 print:w-8">
                                <svg className="h-5 w-5 print:h-4 print:w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round">
                                  <path d="M19 14c1.49-1.46 3-3.21 3-5.5A5.5 5.5 0 0 0 16.5 3c-1.76 0-3 .5-4.5 2-1.5-1.5-2.74-2-4.5-2A5.5 5.5 0 0 0 2 8.5c0 2.3 1.5 4.05 3 5.5l7 7Z" />
                                </svg>
                              </span>
                              ...
                              <div className="min-w-0">
                                <h4 className="truncate text-base font-bold text-slate-900 print:text-sm">
                                  {displayValue(record.type)}
                                </h4>

                                <div className="mt-2 flex flex-wrap items-center gap-2">
                                  {/* Result date chip */}
                                  <span className="inline-flex items-center gap-1.5 rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700 print:bg-white print:ring-1 print:ring-slate-200">
                                    ...
                                    <span className="font-bold">
                                      {record.resultDate && !isNaN(new Date(record.resultDate + "T12:00:00").getTime())
                                  ...
                                  {/* Lab chip */}
                                  {record.lab && (
                                    <span className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-50 px-2.5 py-1 text-xs font-semibold text-indigo-700 ring-1 ring-indigo-100 print:bg-white print:ring-1 print:ring-slate-200 print:text-slate-700">
                            ...
                            {/* Added badge */}
                            <span className="inline-flex shrink-0 items-center gap-1.5 self-start rounded-full bg-slate-50 px-3 py-1 text-[11px] font-medium text-slate-500 ring-1 ring-slate-200/70 print:bg-transparent print:ring-0 print:text-slate-600">
                          ...
                          {/* Remarks */}
                          <div className="mt-4 rounded-xl border border-slate-200/70 bg-slate-50/70 px-4 py-3.5 print:bg-white print:border-slate-200">
                          ...
                          {record.photos && record.photos.length > 0 && (
                            <div className="mt-5">
                              <div className="mb-3 flex items-center gap-2">
                                ...
                                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-400">
                                  Attached Photos · {record.photos.length}
                                </p>
                              </div>
                              ...
                              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 print:grid-cols-3">
                                {record.photos.map((photo, index) => (
                                  <div
                                    key={index}
                                    className="group/img relative aspect-square overflow-hidden rounded-xl bg-slate-100 ring-1 ring-slate-200 transition hover:ring-2 hover:ring-teal-400 print:ring-1 print:ring-slate-200"
                                  >
                                    <img
                                      src={photo}
                                    ...
                                    />
                                    <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-slate-900/40 via-transparent to-transparent opacity-0 transition group-hover/img:opacity-100" />
                                  </div>
                                ))}
                              </div>
                            </div>
